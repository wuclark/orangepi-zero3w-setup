...
- `sudo make board-validation` runs available GPU, Vulkan compute, VPU decode,
  ...
  and the exact prior step that provides it.
...
(`openbox`, `xfce`, `i3`, `icewm`, `fluxbox`, `mate`, `plasma`, `lxqt`, `lxde`) and Wayland sessions
(`sway`, `labwc`):
...
build the external Docker image. For the nested official archive currently in
`work/images`, run `make npu-acuity-image-load`; follow
`docs/optional/npu.md` for the explicit image-version override required before
golden generation.
