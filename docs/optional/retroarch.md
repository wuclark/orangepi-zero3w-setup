...
The default interval between iterations is zero, so the workloads run
continuously for the selected duration. Use `STABILITY_INTERVAL_SECONDS=5` to
insert a pause between iterations; a nonzero interval reduces sustained load
and heat.
