...
| Remove desktop profiles through the installer, never the active one | `--remove` drops profile packages plus orphans and the session file but keeps shared LightDM; refusing the active session prevents stranding the boot target on a half-removed desktop. |
...
| Record every board layer as a replayable manifest receipt | Reinstall overwrites the same step key, so the manifest is current state, not history; replay commands run verbatim in rank order, secrets are never stored, and uninstalls delete keys. New installers must hook in per the data-lifecycle rules. |
| Fall back to XTEST injection where the vendor kernel lacks uinput | `6.6.98-vendor-sun60iw2` ships no uinput module at all, so the installer auto-selects an X11-only XTEST backend (`python3-xlib`, LightDM root authority) there; uinput stays the default wherever `/dev/uinput` is usable. |
| Fire the touch right-click on lift, not mid-hold | A mid-hold fire leaves a dangling press whose release dismisses the fresh menu or triggers the item beneath; firing on lift lets the press complete first so the next tap selects normally. |
| Detect two-finger tap natively instead of packaging touchegg | touchegg is unavailable in trixie and would add a per-session daemon; the existing watcher already sees slot events, so a second live contact disarms the hold path and a quick two-contact chord fires once. |
| Keep --move-units, add --move-fraction as opt-in | Changing units' meaning would silently retune existing installs and replays; the fraction derives from the larger axis span at startup, is logged, and stays a heuristic (equal fractions cover different millimeters on different-sized panels). |
| Evaluate touch drift at SYN frame boundaries | BTN_TOUCH precedes coordinates and axes arrive separately, so anchors form on the first complete pair; per-event evaluation mistook arrival order for finger motion and cancelled every gesture on hardware. |
| Ship hid-multitouch as a pinned out-of-tree module, not a kernel rebuild | Upstream v6.6.98 source matches the vendor kernel ABI (vermagic-checked at install); SHA-256 pins plus an exact-kernel guard keep it narrow, and the module lives in updates/ so the stock kernel is untouched. |
