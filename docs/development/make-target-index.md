...
| `board-vpu-*` | `scripts/board-acceleration-workflow.sh`, `test-vpu-decode.sh`, `test-vpu-quality.sh`, `test-vpu-decode-speed.sh` | Board | Install and validate Cedar/libcedarc, H.264/H.265 decode, Cedar-vs-software PSNR/SSIM quality, and decode speed. |
...
| `desktop-*`, `switch-*`, `desktop-remove`, `remote-*` | `scripts/setup-desktop.sh`, `setup-remote.sh` | Board | Select, remove, or switch desktop or remote backend; removal refuses the active session and keeps LightDM; changes services and graphical targets. |
